...
const jwksUrl = 'https://dev-pjrpl5pms4tnq34t.us.auth0.com/.well-known/jwks.json'
...
/**
 * get keys from Signing Certificate.
 * @param keys : any
 * @param kid : string
 * @returns 
 */
async function getSigninKeys(keys, kid:string) {
  const signinKeys = keys
      ...
      .map(key => {
        return {
          kid: key.kid,
          nbf: key.nbf,
          publicKey: createSigningCertificate(key.x5c[0])
        };
      });
 ...
 * @returns 
 */
function createSigningCertificate(cert) {
